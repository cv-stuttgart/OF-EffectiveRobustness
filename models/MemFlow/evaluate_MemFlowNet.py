...
def validate_sintel(model, cfg, rank=0):
    ...
    for dataset_pass in ['final', "clean"]:
        val_dataset = datasets.MpiSintelTrain(dataset_pass=dataset_pass, return_gt=True)
        print('val_dataset', len(val_dataset), dataset_pass)
        print('val_dataset', [x.shape for x in val_dataset[0] if not isinstance(x, str)])
        ...
        print("Validation (%s) EPE: %f, 1px: %f, 3px: %f, 5px: %f" % (dataset_pass, epe, px1, px3, px5))
        results[dataset_pass] = epe
...
def validate_things(model, cfg, rank=0):
    ...
    for dataset_pass in ['frames_finalpass', "frames_cleanpass"]:
        val_dataset = datasets.ThingsTEST(dataset_pass=dataset_pass, return_gt=True)
        val_loader = data.DataLoader(val_dataset, batch_size=1, pin_memory=True, shuffle=False, num_workers=2)
...
def create_sintel_submission(model, cfg, output_path='output', rank=0):
    ...
    for dataset_pass in ['final', 'clean']:
        test_dataset = datasets.MpiSintel_submission(dataset_pass=dataset_pass, return_gt=False)
        test_sampler = DistributedSampler(test_dataset, num_replicas=cfg.world_size, rank=rank, shuffle=False)
        ...
        for item in tqdm(test_loader):
            ...
            for ti in range(images.shape[1] - 1):
                ...
                if not os.path.exists(f'vis_sintel/{output_path}/{dataset_pass}/flow/{sequence}'):
                   os.makedirs(f'vis_sintel/{output_path}/{dataset_pass}/flow/{sequence}')
                if not os.path.exists(f'vis_sintel/gt/{dataset_pass}/image/{sequence}'):
                   os.makedirs(f'vis_sintel/gt/{dataset_pass}/image/{sequence}')

                image.save(f'vis_sintel/{output_path}/{dataset_pass}/flow/{sequence}/{ti}.png')
                imageio.imwrite(f'vis_sintel/gt/{dataset_pass}/image/{sequence}/{ti}.png', ((images[0, ti].cpu().permute(1, 2, 0).numpy() + 1) * 127.5).astype(np.uint8))

                output_dir = os.path.join('sintel_submission', output_path, dataset_pass, sequence)
