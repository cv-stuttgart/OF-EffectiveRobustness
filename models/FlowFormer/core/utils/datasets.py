...
from models.FlowFormer.core.utils import frame_utils
from models.FlowFormer.core.utils.augmentor import FlowAugmentor, SparseFlowAugmentor
#from models.FlowFormer.core.utils import flow_transforms 

from torchvision.utils import save_image

from models.FlowFormer.core.utils import flow_viz
import cv2
from models.FlowFormer.core.utils.utils import coords_grid, bilinear_sampler
...
class MpiSintel_submission(FlowDataset):
    def __init__(self, aug_params=None, split='test', root='datasets/Sintel', dataset_pass='clean'):
...
class MpiSintel(FlowDataset):
    def __init__(self, aug_params=None, split='training', root='datasets/Sintel', dataset_pass='clean'):
        super(MpiSintel, self).__init__(aug_params)

        root = 's3://'

        self.image_list = []
        with open("./flow_dataset/Sintel/Sintel_"+dataset_pass+"_png.txt") as f:
            images = f.readlines()
            for img1, img2 in zip(images[0::2], images[1::2]):
                self.image_list.append([root+img1.strip(), root+img2.strip()])
        
        self.flow_list = []
        with open("./flow_dataset/Sintel/Sintel_"+dataset_pass+"_flo.txt") as f:
            flows = f.readlines()
            for flow in flows:
        ...
        self.extra_info = []
        with open("./flow_dataset/Sintel/Sintel_"+dataset_pass+"_extra_info.txt") as f:
            info = f.readlines()
...
class FlyingThings3D(FlowDataset):
    def __init__(self, aug_params=None, root='datasets/FlyingThings3D', dataset_pass='frames_cleanpass'):
        super(FlyingThings3D, self).__init__(aug_params)

        root = 's3://'

        self.image_list = []
        with open("./flow_dataset/flying_things/flyingthings_"+dataset_pass+"_png.txt") as f:
            images = f.readlines()
            for img1, img2 in zip(images[0::2], images[1::2]):
                self.image_list.append([root+img1.strip(), root+img2.strip()])
        self.flow_list = []
        with open("./flow_dataset/flying_things/flyingthings_"+dataset_pass+"_pfm.txt") as f:
            flows = f.readlines()
            for flow in flows:
                self.flow_list.append(root+flow.strip())
        ...
        #         image_dirs = sorted(glob(osp.join(root, dataset_pass, 'TRAIN/*/*')))
...
def fetch_dataloader(args, TRAIN_DS='C+T+K+S+H'):
    ...
    if args.stage == 'chairs':
        ...
        clean_dataset = FlyingThings3D(aug_params, dataset_pass='frames_cleanpass')
        final_dataset = FlyingThings3D(aug_params, dataset_pass='frames_finalpass')
        train_dataset = clean_dataset + final_dataset

    elif args.stage == 'sintel':
        aug_params = {'crop_size': args.image_size, 'min_scale': -0.2, 'max_scale': 0.6, 'do_flip': True}
        things = FlyingThings3D(aug_params, dataset_pass='frames_cleanpass')
        sintel_clean = MpiSintel(aug_params, split='training', dataset_pass='clean')
        sintel_final = MpiSintel(aug_params, split='training', dataset_pass='final')        
