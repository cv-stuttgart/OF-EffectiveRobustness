...
class RAFTGMA(nn.Module):
    ...
    def forward(self, image1, image2, iters=12, flow_init=None, upsample=True, test_mode=False, **kwargs):
...
class RAFTGMA_QuadtreeFnet(RAFTGMA):
    def __init__(self, args):
        super(RAFTGMA_QuadtreeFnet, self).__init__(args)
        self.args = args

        # feature network, context network, and update block
        self.fnet = MatchingModel(cfg=args)
        self.fnet = self.fnet.load_state_dict(path=args.matching_model_path)