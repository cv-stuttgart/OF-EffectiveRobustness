...
def create_sintel_submission(model,
    ...
    model.eval()
    for dataset_pass in ['clean', 'final']:
        test_dataset = MpiSintel(split='test', aug_params=None, dataset_pass=dataset_pass)
        ...
        for test_id in range(len(test_dataset)):
            ...
            output_dir = os.path.join(output_path, dataset_pass, sequence)
...
def validate_things(model,
    ...
    for dataset_pass in ['frames_cleanpass', 'frames_finalpass']:
        if val_things_clean_only:
            if dataset_pass == 'frames_finalpass':
                continue

        val_dataset = FlyingThings3D(dataset_pass=dataset_pass, test_set=True, validate_subset=True,
                                     )
        print('Number of validation image pairs: %d' % len(val_dataset))
        epe_list = []
        ...
        if dataset_pass == 'frames_cleanpass':
            dataset_pass = 'things_clean'
        if dataset_pass == 'frames_finalpass':
            dataset_pass = 'things_final'

        print("Validation Things test set (%s) EPE: %.3f" % (dataset_pass, epe))
        results[dataset_pass + '_epe'] = epe
        ...
        if with_speed_metric:
            ...
            results[dataset_pass + '_s0_10'] = s0_10
            results[dataset_pass + '_s10_40'] = s10_40
            results[dataset_pass + '_s40+'] = s40plus
...
def validate_sintel(model,
    ...
    for dataset_pass in ['clean', 'final']:
        val_dataset = MpiSintel(split='training', dataset_pass=dataset_pass,
        ...
        dataset_pass_ori = dataset_pass

        print("Validation Sintel (%s) EPE: %.3f, 1px: %.3f, 3px: %.3f, 5px: %.3f" % (dataset_pass_ori, epe, px1, px3, px5))

        dataset_pass = 'sintel_' + dataset_pass

        results[dataset_pass + '_epe'] = np.mean(epe_list)
        results[dataset_pass + '_1px'] = px1
        results[dataset_pass + '_3px'] = px3
        results[dataset_pass + '_5px'] = px5
        ...
        if evaluate_matched_unmatched:
            ...
            print('Validatation Sintel (%s) matched epe: %.3f, unmatched epe: %.3f' % (
                dataset_pass_ori, matched_epe, unmatched_epe))

            results[dataset_pass + '_matched'] = matched_epe
            results[dataset_pass + '_unmatched'] = unmatched_epe
