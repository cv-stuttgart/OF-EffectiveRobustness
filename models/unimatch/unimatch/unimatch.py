...
class UniMatch(nn.Module):
    ...
    def forward(self, img0, img1,
        ...
        if task != 'depth':
            assert len(attn_splits_list) == len(corr_radius_list) == len(prop_radius_list) == self.num_scales, (len(attn_splits_list), len(corr_radius_list), len(prop_radius_list), self.num_scales)
        else:
