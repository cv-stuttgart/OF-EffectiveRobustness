    ...
    def __init__(self, aug_params=None,
        ...
        for cam in ['left']:
            for direction in ['into_future', 'into_past']:
                if test_set:
                    image_dirs = sorted(glob(osp.join(img_dir, dataset_pass, 'TEST/*/*')))
                else:
                    image_dirs = sorted(glob(osp.join(img_dir, dataset_pass, 'TRAIN/*/*')))
...
def build_train_dataset(args):
    if args.stage == 'chairs':
        ...
        things = FlyingThings3D(aug_params, dataset_pass='frames_cleanpass')  # 40302
