...
class MpiSintel(FlowDataset):
    def __init__(self, aug_params=None, split='training', root='/data/Sintel', dataset_pass='clean',
...
class FlyingChairs(FlowDataset):
    def __init__(self, aug_params=None, split='training', root='/data/FlyingChairs_release/data'):
        ...
        split_list = np.loadtxt(root[:-4]+'FlyingChairs_train_val.txt', dtype=np.int32)
...
class FlyingThings3D(FlowDataset):
    def __init__(self, aug_params=None, root='/data/FlyingThings3D', split='training', dataset_pass='frames_cleanpass'):
        ...
        if split == 'training':
            ...
            for cam in ['left']:
                for direction in ['into_future', 'into_past']:
                    image_dirs = sorted(glob(osp.join(root, dataset_pass, 'TEST/*/*')))
                    image_dirs = sorted([osp.join(f, cam) for f in image_dirs])
...
class KITTI(FlowDataset):
    def __init__(self, aug_params=None, split='training', root='/data/kitti15'):
...
def fetch_dataloader(args, TRAIN_DS='C+T+K+S+H'):
    ...
    if args.stage == 'chairs':
        ...
        train_dataset = FlyingChairs(aug_params, split='training', root=args.chairs_root)
    
    elif args.stage == 'things':
        aug_params = {'crop_size': args.image_size, 'min_scale': -0.4, 'max_scale': 0.8, 'do_flip': True}
        clean_dataset = FlyingThings3D(aug_params, dataset_pass='frames_cleanpass', split='training', root=args.things_root)
        final_dataset = FlyingThings3D(aug_params, dataset_pass='frames_finalpass', split='training', root=args.things_root)
        train_dataset = clean_dataset + final_dataset

    elif args.stage == 'sintel':
        aug_params = {'crop_size': args.image_size, 'min_scale': -0.2, 'max_scale': 0.6, 'do_flip': True}
        things = FlyingThings3D(aug_params, dataset_pass='frames_cleanpass', root=args.things_root)
        sintel_clean = MpiSintel(aug_params, split='training', dataset_pass='clean', root=args.sintel_root)
        sintel_final = MpiSintel(aug_params, split='training', dataset_pass='final', root=args.sintel_root)        

        if TRAIN_DS == 'C+T+K+S+H':
            kitti = KITTI({'crop_size': args.image_size, 'min_scale': -0.3, 'max_scale': 0.5, 'do_flip': True}, root=args.kitti_root)
            hd1k = HD1K({'crop_size': args.image_size, 'min_scale': -0.5, 'max_scale': 0.2, 'do_flip': True}, root=args.hd1k_root)
        ...
        train_dataset = KITTI(aug_params, split='training', root=args.kitti_root)
