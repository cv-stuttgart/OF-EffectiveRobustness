...
def validate_things(model, iters=6, root='/data/FlyingThings3D'):
    ...
    for dataset_pass in ['frames_cleanpass', 'frames_finalpass']:
        epe_list = []
        val_dataset = datasets.FlyingThings3D(dataset_pass=dataset_pass, split='validation', root=root)
        ...
        results[dataset_pass] = np.mean(epe_list)
...
def validate_sintel(model, iters=6, root='/data/Sintel', tqdm_miniters=1):
    ...
    for dataset_pass in ['clean', 'final']:
        val_dataset = datasets.MpiSintel(split='training', dataset_pass=dataset_pass, root=root)
        epe_list = []
...
def validate_sintel_occ(model, iters=6, root='/data/Sintel', tqdm_miniters=1):
    ...
    model.eval()
    results = {}
    for dataset_pass in ['albedo', 'clean', 'final']:
    # for dataset_pass in ['clean', 'final']:
        val_dataset = datasets.MpiSintel(split='training', dataset_pass=dataset_pass, occlusion=True)
        epe_list = []
        epe_occ_list = []
        epe_noc_list = []
